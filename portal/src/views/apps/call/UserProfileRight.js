                  ...
                  badgeContent={
                    <Box
                      ...
                      sx={{
                        ...
                        height: 10,
                        borderRadius: '50%',
                        color: `${statusObj[store.selectedCall.contact.status]}.main`,
                        boxShadow: theme => `0 0 0 2px ${theme.palette.background.paper}`,
                        backgroundColor: `${statusObj[store.selectedCall.contact.status]}.main`
                      }}
                    />
                  }
                >
                  {store.selectedCall.contact.avatar ? (
                    <MuiAvatar
                      sx={{ width: '5rem', height: '5rem' }}
                      src={store.selectedCall.contact.avatar}
                      alt={store.selectedCall.contact.fullName}
                    />
                  ) : (
                    <CustomAvatar
                      skin='light'
                      color={store.selectedCall.contact.avatarColor}
                      sx={{ width: '5rem', height: '5rem', fontWeight: 500, fontSize: '2rem' }}
                    >
                      {getInitials(store.selectedCall.contact.fullName)}
                    </CustomAvatar>
                  )}
                </Badge>
              </Box>
              <Typography sx={{ mb: 0.5, fontWeight: 500, textAlign: 'center' }}>
                {store.selectedCall.contact.fullName}
              </Typography>
              <Typography variant='body2' sx={{ textAlign: 'center' }}>
                {store.selectedCall.contact.role}
              </Typography>
            </Box>
          </Box>
            ...
            <ScrollWrapper>
              <Box sx={{ p: 5 }}>
                <FormGroup sx={{ mb: 10.5 }}>
                  ...
                  <Typography sx={{ fontSize: '0.875rem' }}>{store.selectedCall.contact.about}</Typography>
                </FormGroup>
                  ...
                  <List dense sx={{ p: 0 }}>
                    <ListItem sx={{ px: 2 }}>
                      ...
                      <ListItemText
                        ...
                        primary={`${store.selectedCall.contact.fullName.replace(/\s/g, '_')}@email.com`}
                      />
                    </ListItem>
                    <ListItem sx={{ px: 2 }}>
                      <ListItemIcon sx={{ mr: 2, color: 'text.primary' }}>
